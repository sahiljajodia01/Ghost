...
public class TrieNode {
    ...
    public String getGoodWordStartingWith(String s) {
        ...
        ArrayList<Character> charsAreWords = new ArrayList<>();
        ArrayList<Character> charsAreNoWords = new ArrayList<>();
        Character c;

        while (true) {
            charsAreWords.clear();
            charsAreNoWords.clear();

            for(Character ch: temp.children.keySet()) {
                if(temp.children.get(ch).isWord) {
                    charsAreWords.add(ch);
                }
                else {
                    charsAreNoWords.add(ch);
                }
            }

            if(charsAreNoWords.size() == 0) {
                s += charsAreWords.get(random.nextInt(charsAreWords.size()));
                break;
            }
            else {
                c = charsAreNoWords.get(random.nextInt(charsAreNoWords.size()));
                s += c;
                temp = temp.children.get(c);
            }
        }
